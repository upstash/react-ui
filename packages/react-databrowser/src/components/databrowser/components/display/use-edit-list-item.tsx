...
export const useEditListItem = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (type === "set") {
        if (shouldDelete) pipe.srem(dataKey, itemKey);
        if (newKey) pipe.sadd(dataKey, newKey);
      } else if (type === "zset") {
        ...
        if (Number.isNaN(index)) throw new Error("Index must be a number for list data type");
